...
def waste_segregator(img):
    ...
    size = (224, 224)  # Make sure this matches your model's input size
    ...
    class_name = class_names[index].strip()  # Remove any extra whitespace/newline
...
try:
    ...
    class_names = [name.strip() for name in class_names]  # Clean whitespace/newlines
except Exception as e:
    print(f"Error loading labels: {e}")
    st.error("Failed to load class names. Please check the labels file.")
